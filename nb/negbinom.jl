...
n, gene = open(counts, "../data/raw/dge_raw.txt"); size(n)
...
We fit a GLM negative-binomial model to each gene.
"""
...
let
	...
	depth = vec(sum(n,dims=1))
	level = vec(sum(n,dims=2))
	
	lines!(ax1, sort(depth), range(0,1,size(n,2)),
		linewidth=2,
		color=:black
	)
	
	lines!(ax2, sort(level), range(0,1,size(n,1)),
		linewidth=2,
		color=:black
	)
...
let
	χ = [ gene |> vec |> Normalize.logmean for gene in eachrow(n) ]
	...
	hlines!(ax1, [1e1], color=:red, linewidth=4, linestyle=:dashdot)
	fig
end
...
let
	...
	scatter!(ax1, model.Θ₂, model.δΘ₂, color=log.(χ), colormap=:inferno)
	vlines!(ax1, 1, linewidth=4, linestyle=:dashdot, color=:cyan)
	fig
end
...
let
	...
	scatter!(ax1, model.Θ₃, model.δΘ₃, color=log.(χ), colormap=:inferno)
	hlines!(ax1, [50], color=:black, linewidth=3, linestyle=:dashdot)
	vlines!(ax1, [50], color=:black, linewidth=3, linestyle=:dashdot)
	fig
end
...
let
	...
	for g in findall(model.δΘ₃ .> 1e5)
		lines!(ax1,
			sort(vec(n[g,:])), range(0,1,size(n,2)),
			color=(:black,.5),
		)
	end
	fig
...
function filter_uncertain(count, gene, model; cutoff=50)
	...
	keep = findall(
		(uncertainty .<= cutoff) .&
		(vec(sum(count .== 0, dims=2)) .<= .95*size(count,2))
	)
...
N, G, Model = filter_uncertain(n, gene, model; cutoff=10); size(N)
...
bootstrap = let
	χ = [ cell |> vec |> Normalize.logmean for cell in eachcol(N) ]
	[ Normalize.bootstrap(vec(N[i,:]), χ) for i in 1:10:size(N,1) ]
end;
...
μ, σ², u, v = let
	σ² = N.*(N.+Model.Θ₃) ./ (1 .+ Model.Θ₃)
	u², v², _ = Utility.sinkhorn(σ²)

	μ  = Diagonal(.√u²)*N*Diagonal(.√v²)
...
gmodel = Normalize.glm(μ̃.+1e-8; stochastic=Normalize.gamma, ϵ=1e-8);
...
let
	...
	for i in 1:10:size(gmodel.residual,1)
		χ = .5*(1 .+ erf.( (gmodel.residual[i,:] |> vec) ./ sqrt(2)))
		lines!(ax1, sort(χ), range(0,1,length(χ)), color=(:black,.01))
	end
	fig
end
...
μ̂ = let
	F = svd(μ)
	μ = F.U[:,2:30]*Diagonal(F.S[2:30])*F.Vt[2:30,:]
...
let
	...
	F = svd(μ)
	scatter!(ax1, sum(μ,dims=1) |> vec, -F.Vt[1,:]|> vec)
	scatter!(ax2, sum(μ,dims=2) |> vec, -F.U[:,1]|> vec)
...
=#
	.5*(1 .+ erf.(gmodel.residual ./ sqrt(2)))
	#gmodel.residual
end;
...
# ╔═╡ 74936204-2185-4801-b65f-0e6ced669aa3
begin
	import Match
	import Roots
	import GZip, JLD2, FileIO
	import StatsFuns, Distributions, BlackBoxOptim
end
...
database = Inference.virtualembryo(directory="/home/nolln/src/seqspace/data/raw");

# ╔═╡ 98506181-1e09-432e-9e47-68eeda650507
inv_z = Inference.inversion(Z, G; refdb=database); inv_m = Inference.inversion(μ̂, G; refdb=database);

# ╔═╡ 5b74e210-9b3d-422c-8e89-525ccdd17c47
ψz = minimum(size(N))*inv_z.invert(0.1); ψm = minimum(size(N))*inv_m.invert(0.1);
...
let
	...
	scatter!(ax3,
		-database.position[:,1],database.position[:,2],database.position[:,3],
		color=ψm*vec(μ[findfirst(G.=="twi"),:]),
		colormap=:inferno,
		markersize=2000,
	)
	hidedecorations!(ax3)
...
Xz = ψz'*database.position; Xm = ψm'*database.position;
...
# ╔═╡ 0e4c49c2-3c5c-458c-a896-73b4c04f3582
...
Dz = PointCloud.geodesics(Z,12); Dm = PointCloud.geodesics(μ̂,12);
...
ξz = PointCloud.isomap(Z, 10); ξm = PointCloud.isomap(μ̂, 10); size(ξz)
...
let
	...
	scatter!(ax1, ξz[:,1], -ξz[:,2], ξz[:,3], markersize=4000,color=Xz[:,1], colormap=:inferno)
	scatter!(ax2, ξz[:,1], -ξz[:,2], ξz[:,3], markersize=4000,color=atan.(Xz[:,2],Xz[:,3]), colormap=:inferno)
	fig
end
...
let
	...
	scatter!(ax2, ξm[:,1], ξm[:,2], -ξm[:,3], markersize=4000, color=atan.(Xm[:,2],Xz[:,3]), colormap=:inferno)
	fig
end
...
[[deps.NMF]]
deps = ["LinearAlgebra", "NonNegLeastSquares", "Printf", "Random", "RandomizedLinAlg", "Statistics", "StatsBase"]
git-tree-sha1 = "bb6043f8bffb1437128b3d1c036801c624f9220b"
